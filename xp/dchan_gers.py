...
def build_params(stage: str, root_dir: str, fold_nb: int, batch_size: int, num_workers: int, transform: Optional[DualTransform] = None, nb_samples: int = 0, sample_seed = 0) -> Dict:
    ...
    res = {
        'input_fields': {
            ...
            "mask": {"name": "change", "type": "mask", "dtype": "uint8"},            
        },
        'dataloader_options' : {"batch_size": batch_size, "num_workers": num_workers},
        'input_file': dataset,
        'root_dir': root_dir,
...
def main():
    ...
    mlflow_experiment_name = "dchan_levir"
    ...
    train_dropout: Optional[float] = None
    if 'TRAIN_DROPOUT' in os.environ:
        train_dropout = float(os.environ['TRAIN_DROPOUT'])
