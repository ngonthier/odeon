...
def main():
    ...
    if clearml:
        clearml_experiment_name = "dchan_hiucd"
        if 'CLEARML_EXPERIMENT_NAME' in os.environ:
            clearml_experiment_name = os.environ['CLEARML_EXPERIMENT_NAME']

    tensorboard_location = "/mnt/stores/store-DAI/pocs/slurm/dchan/tensorboard_ng"
    ...
    if clearml:
        print("clearml_experiment_name", clearml_experiment_name)
        print("clearml_run_name", name)
    print("lr", train_lr)
    ...
    if trainer.global_rank == 0 and has_log:
        ...
        if clearml:
            ...
            if encoder_weights is not None:
                clearm_tags.append(encoder_weights)
            task.add_tags(clearm_tags)
            task.set_configuration_object("fit_params", config_type="datamodule", config_dict=fit_params)
            task.set_configuration_object("val_params", config_type="datamodule", config_dict=val_params)
