...
if not os.path.exists(folder_path):
    ...
    if not os.path.exists(folder_path):
        folder_path = r'/mnt/stores/store-DAI/datasrc/dchan/hiucd_mini' #\train\image\2017\9"
...
for split in ['train','val','test']: 
    ...
    df = pd.DataFrame(columns=["id", "id_zone","millesime1","millesime2","change_pat", "T0_path", "T1_path", "change_path", "T0", "T1", "change"])
    ...
    for file in files:
        ...
        change_value = os.path.join(split,"mask_merge",year_0+'_'+year_1,"9", file)
        
        # Append a new row to the DataFrame
        df.loc[len(df)] = {"id": id_value,"id_zone": id_zone, "millesime1": year_0, "millesime2": year_1, "T0_path": T0_path_value, "T1_path": T1_path_value, "change_path": change_value,
